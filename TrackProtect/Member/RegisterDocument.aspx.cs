...
{
    public partial class RegisterDocument : BasePage
    {
        #region Variables
        ...
        private void RedirectToManaccPage()
        {
            if (Request.QueryString.Count > 0 && Request.QueryString["Mode"] == "Self")
            {
            }
            else
            {
                ...
                {
                    Response.Redirect("RegisterDocSelection.aspx");
                }
            }
        }
        ...
        private string CreateCertificate
           (long userid, string password, string doc1, string doc2, string doc3, string isrcCode, string trackName, CoopArtistList coopArtists)
        {
        ...
        private long RegisterDoc(long registerid, string documentname)
        {
            ...
            using (Database db = new MySqlDatabase())
            {
                ...
                string path = "/" + strArray[strArray.Length - 1];

                documentid = db.RegisterDocument(registerid, path, documenthash);
            }
            return documentid;
        }
        ...
        protected void SubmitButton_Command(object sender, CommandEventArgs e)
        {
            ...
            if (Page.IsValid)
            {
                RegisterDocumentButton.Enabled = false;

                string facebookResponse = string.Empty;
                string twitterResponse = string.Empty;
                string soundCloudResponse = string.Empty;

                bool failed = false;
                long userid = Util.UserId;
                if (userid == 0)
                {
                    // Oops, something is wrong no userid known.
                    // Report it and bail out
                ...
                using (Database db = new MySqlDatabase())
                {
                    ...
                    if (!failed)
                    {
                        ...
                        string certfilename = CreateCertificate(userid, password, f1, f2, f3, isrcCode, trackName,
                                                                coopArtists);
                        registerid =
                            db.CreateRegistry
                            (userid, certfilename, f1, trackName.Trim(), isrcCode,
                        ...
                        if (registerid > 0)
                        {
                            ...
                            if (!failed)
                            {
                                db.DecrementCredits(userid);
                                db.AddCreditHistory(userid, 0L, 1, 0);

                                string certdocfile = Path.ChangeExtension(certfilename, ".pdf");
                                Util.SendRegistration(Util.UserId, uploadPath, trackName, certfilename, certdocfile);
                            }
                            else
                            {
                    ...
                    {
                        // Delete the files if upload failed
                ...
                if (failed == false)
                {
                    ...
                    else if (cbxSendToSoundCloud.Checked && File.Exists(Path.Combine(uploadPath, doc1)))
                    {
                        ...
                        Session["SoundCloud.Response"] =
                            Publish.SoundCloudPublish(Convert.ToString(Session["soundCloudAccessToken"]),
                            Path.Combine(uploadPath, filename), trackName + System.Environment.NewLine + SoundCloudMsg.Text.Trim());
                    }
                        ...
                        if (!Convert.ToString(Session["SoundCloud.Response"]).Contains("Deactive") && !Convert.ToString(Session["SoundCloud.Response"]).Contains("Error"))
                        {
                            ...
                            else if (cbxSendToFacebook.Checked)
                            {
                                ...
                                Session["Facebook.Response"] =
                                    postToFacebook(messageToPost, stream_url, Convert.ToString(Session["FBACC"]), Convert.ToString(Session["FBID"]));
                ...
                string continueUrl = "~/Member/UploadConfirmation.aspx?type=own";
    ...
    // Custom validatore code behind validation event
